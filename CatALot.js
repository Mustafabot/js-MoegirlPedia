            ...
            getTargetCat: function ( pages, targetcat, mode ) {
                if ( !this.cancelled ) {
                    this.doAPICall( {
                        ...
                        // 并发控制参数
                        const concurrencyLimit = mw.config.get('wgUserGroups').includes('bot') ? 5 : 1;
                        const delay = mw.config.get('wgUserGroups').includes('bot') ? 200 : 21000;
                        ...
                        const processQueue = (index) => {
                            ...
                            if (activeRequests < concurrencyLimit) {
                                activeRequests++;
                                return CAL.getContent(pages[index], targetcat, mode)
                                    .then(() => {
                                        activeRequests--;
                                        return timer(delay).then(() => processQueue(index + 1));
                                    });
                            } else {
                                return timer(100).then(() => processQueue(index));
                            }
                        };
                        
                        // 启动并发处理
                        for (let i = 0; i < concurrencyLimit; i++) {
                            promises.push(processQueue(i));
                        }
