//<pre> 批量挂删页面工具，逐个挂删，改造自[[User:星海子/js/MassDelete.js]]以适应机器人（bot）使用。
"use strict";
mw.loader.using(["mediawiki.api", "mediawiki.Title", "ext.gadget.libOOUIDialog"], () => {
    ...
    function MassDeleteForm() {
        ...
        document.getElementById(bodyContent).innerHTML = "<p>大量挂删页面之工具，请慎用！</p>" +
            ...
            "<option>内容极少或质量极差</option>" +
            "<option>用户本人申请</option>" +
            "<option>错误命名</option>" +
            "<option>移动残留重定向</option>" +
            "<option>无法修复的受损重定向</option>" +
            "<option>无使用或不再使用的文件</option>" +
            "<option>删除以便重新上传</option>" +
            "<option>存在重复或有更佳版本的文件</option>" +
            "<option>不在收录范围</option>" +
