...
//DisamAssist，萌百修改版
//DisamAssist的使用方法等请见[[User:没有羽翼的格雷塔/js#DisamAssist]],反馈请到[[User talk:没有羽翼的格雷塔]]。

window.DisamAssist = jQuery.extend( true, {
	cfg: {
		/*
		 * 消歧义页的分类（通常通过[[Template:消歧义页]]添加）（目前有bug，需要简繁体一起添加，否则可能不加载）
		 */
		disamCategories: ['消歧义页','消歧義頁'],
		...
		/*
		 * 一次性查询的标题数量（查消歧义页内链接的重定向关系的，没啥用，有空删了）
		 */
		queryTitleLimit: 1,
		...
		/*
		 *提交编辑的最短时间间隔，以秒为单位。使用负值或0则不会进行冷却。拥有“bot”权限的用户不会受到冷却时间的影响。（由于存在ratelimit，不建议低于6秒）
		 */
		editCooldown: 20,
	...
	txt: {
		...
		getBacklinksError: '无法获取反向链接: "$1".',
		fetchRightsError: '无法获取用户权限："$1",',
		loadPageError: '无法加载 $1: "$2".',
		savePageError: '无法提交编辑到 $1: "$2".',
		dismissError: '跳过',
		pending: '存在尚未保存的编辑。如欲保存，请按“关闭”。',
		editInProgress: 'DisamAssist正在提交编辑。如果您将该页面关闭，可能会丢失您的编辑。',
		ellipsis: '…',
		notifyCharacter: '✔',
		summary: '使用[[U:没有羽翼的格雷塔/js#DisamAssist|DisamAssist]]清理[[MGP:消歧义方针|消歧义]]链接：[[$1]]（$2）。',
		...
		summarySeparator: '; ',
		redirectSummary: '使用[[U:没有羽翼的格雷塔/js#DisamAssist|DisamAssist]]创建目标为[[$1]]的重定向。'
	}
}, window.DisamAssist || {} );

mw.loader.load('//cdn.jsdelivr.net/gh/Mustafabot/js-MoegirlPedia/DisamAssist-core.min.js');
//</syntaxhighlight>
